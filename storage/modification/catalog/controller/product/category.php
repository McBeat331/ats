...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if (isset($this->request->get['path'])) {
			...
			$category_tpl = array();
			$category_tpl['[name]'] = $category_info['name'];
			$cats = array();
			if (isset($this->request->get['path'])) {
				$parts = explode('_', (string)$this->request->get['path']);
				foreach ($parts as $path_id) {
			...
			if (count($cats) > 1) {
				$category_tpl['[category]'] = $cats[count($cats)-2];
			} else {
		...
		if ($category_info) {
			...
			if ($seogenerator['category_title']) {
				$this->document->setTitle(trim(strtr($seogenerator['category_title'], $category_tpl)));
			} else {
			...
			} else if ($seogenerator['category_description']) {
				$this->document->setDescription(trim(strtr($seogenerator['category_description'], $category_tpl)));
			} else {
			...
			} else if ($seogenerator['category_keywords']) {
				$this->document->setKeywords(trim(strtr($seogenerator['category_keywords'], $category_tpl)));
			} else {
				$this->document->setKeywords($category_info['name']);
			}
			

			
			if ($seogenerator['category_h1']) {
				$data['heading_title'] = (trim(strtr($seogenerator['category_h1'], $category_tpl)));
			} else {
			...
			foreach ($results as $result) {
				...
				if ($this->config->get('config_review_status')) {
					$rating = false;//(int)$result['rating'];
				} else {
					$rating = false;
				}
